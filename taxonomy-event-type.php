<?php
/**
 * Template Name: Custom Taxonomy Page
 * Description: A custom search results page with post-type filter and pagination.
 */

get_header('home');

// Get the taxonomy slug from the query variable
$taxonomy_slug = get_query_var('taxonomy');
$taxonomy_name = "Events";

// Check if a taxonomy slug is found
if ( $taxonomy_slug ) {
...
<main class="quipli main less-spacing">
	...
	echo '<a href="' . home_url() . '/resources/events/">Events</a><span class="separator">›</span>';
	echo '<span>'.single_term_title( "", false ).'</span></div></div>';
	?>
  ...
  <?php 
		...
		$blocks = array(
			'event' => array('title' => single_term_title( "", false ) . ' Events', 'slug' => 'resources/events', 'post_type' => 'events'),
		);
